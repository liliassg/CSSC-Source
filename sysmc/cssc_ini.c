/*
 * cssc_ini.c — v6-native runtime for the cssc.ini CSSC module.
 *
 * Minimal INI file handler that matches the interpreter's semantics:
 *   ini::write(path, section, key, value) -> int (1 on success, 0 on I/O
 *     failure). Overwrites the value if `key` already exists under `section`;
 *     otherwise appends. Creates the section header if missing. Creates the
 *     file if missing.
 *   ini::read(path, section, key, default) -> string (heap cssc_str*).
 *     Returns `default` if the file, section, or key is missing.
 *   ini::has_key(path, section, key) -> int (1/0).
 *   ini::delete(path, section, key) -> int (1/0). Removes the line.
 *   ini::exists(path) -> int (1/0).
 *
 * The parser is line-oriented, permissive on whitespace, comment characters
 * (';' and '#') strip trailing content, and unquoted values are kept
 * verbatim through the trailing newline strip. No section/key nesting;
 * standard flat INI.
 */
...
/* Externally supplied — defined in the transembly runtime and cssc_host_game.c
 * for heap string creation. Signature: (const char*, size_t) -> cssc_str*. */
...
/* Return >0 if `line` starts a section named `section`. */
...
/* Return >0 if `line` is a `key = value` binding matching `key`. */
...
/*
 * Rewrite the file with `key=value` in `[section]`. If the section doesn't
 * exist, append `[section]\nkey=value\n`. If the key exists in the section,
 * overwrite its value. Otherwise append the key line inside the section.
 */
CSSC_INI_EXPORT int64_t cssc_ini_write(const char* path, const char* section,
    ...
    char* buf = NULL;
    size_t buf_len = 0;
    (void)ini_read_all(path, &buf, &buf_len);   /* missing file → empty */
    /* Output buffer with generous headroom for the new binding. */
    ...
    char* line_start = buf;
    /* Iterate the source char-by-char to preserve blank lines / comments. */
    for (char* p = buf; buf; ++p) {
        int at_end = (*p == '\0');
        int at_nl  = (*p == '\n');
        if (!at_end && !at_nl) continue;
        size_t line_len = (size_t)(p - line_start);
        /* Copy the raw line for output later — but first classify it. */
        char* line_copy = (char*)malloc(line_len + 1);
        if (!line_copy) { free(out); free(buf); return 0; }
        memcpy(line_copy, line_start, line_len);
        line_copy[line_len] = '\0';
        /* CR handling: keep the CR on the output but drop it from
         * the classification copy. */
        char cls[1024];
        ...
        /* Strip inline comments for classification only. */
        for (char* c = cls; *c; ++c) {
            if (*c == ';' || *c == '#') { *c = '\0'; break; }
        }
        ...
        if (is_section) {
            /* Entering a new section — if we were in the target section
             * and haven't written the key yet, append it here (before the
             * new section header). */
    ...
    if (!saw_target_section) {
        /* Section doesn't exist — append it and the key. */
        ...
        /* Section existed but key was not present — append the key at EOF
         * (best we can do without tracking the section's byte range). */
...
CSSC_INI_EXPORT int64_t cssc_ini_delete(const char* path, const char* section,
    ...
    for (char* p = buf; ; ++p) {
        ...
        if (*trimmed == '[') {
            ...
            removed = 1;
            /* Also drop the following newline to avoid a stray blank line. */
            free(line_copy);
            if (at_end) break;
            line_start = p + 1;
            continue;
