...
#include <errno.h>
  ...
  #define GetCurrentProcessId _cssc_get_pid
    ...
    #define CSSC_CATCH_STACK_MAX   8
  #endif
  #ifndef CSSC_LAST_ERROR_MAX
    #define CSSC_LAST_ERROR_MAX    256
  #endif
  #ifndef MAX_HEX_VARS
    #define MAX_HEX_VARS           8
  #endif
  #ifndef MAX_HEX_SCOPES
    #define MAX_HEX_SCOPES         4
  #endif
  #ifndef MAX_DEFERRED
...
CSSC_API CsscVal cssc_bind(void) {
    ...
    bind->pairs = (CsscVal*)cssc_alloc(sizeof(CsscVal) * 16);
    CsscVal v;
...
CSSC_API void cssc_retain(CsscVal v) {
    ...
    if (!h) return;
    if (h->refcount == 0xFFFFFFFFu) return;
    h->refcount++;
}
...
CSSC_API void cssc_release(CsscVal v) {
    CsscHeapHeader* h = cssc_get_header(v);
    if (!h) return;
    if (h->refcount == CSSC_REFCOUNT_ARENA) return;
    if (h->refcount <= 1) {
...
CSSC_API CsscVal cssc_builtin_time(void) {
    ...
    time_t t = time(NULL);
    if (!_cssc_has_wallclock(t)) return cssc_int(0);
...
static uint32_t _cssc_diag_min_free = 0xFFFFFFFFu;
static uint32_t _cssc_diag_last_tick_ms = 0;
#ifndef CSSC_DIAG_TICK_MIN_MS
#define CSSC_DIAG_TICK_MIN_MS 1000
#endif
...
static DWORD WINAPI _cssc_video_thread(LPVOID param) {
    ...
    bmi.bmiHeader.biHeight = -(LONG)args->height;
...
CSSC_API CsscVal cssc_video_create(int64_t width, int64_t height, int64_t fps) {
    ...
    v->header.refcount = 1;
    v->header.type = CSSC_TYPE_MODULE;
...
CSSC_API CsscVal cssc_obj_load(CsscScopeStack* host_scope, const char* obj_path, const char* alias) {
    ...
    handle->project_name = cssc_intern(alias);
...
#ifdef _WIN32
static volatile LONG g_watermark_shown = 0;
...
#ifdef _WIN32
    if (InterlockedCompareExchange(&g_watermark_shown, 1, 0) != 0) return;
...
static CsscScopeStack* g_external_scope = NULL;
